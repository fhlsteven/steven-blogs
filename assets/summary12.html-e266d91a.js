...
</code></pre></div><p>有许多原因会使代码引发异常，如文件不存在，访问权限不够等等。在现在这种没有异常处理的情况下，运行时发生的任何错误会回溯到.NET运行时(CLR)；而运行时会呈现给用户一个让人费解并可能造成危险的对话框（图1）。为了避免出现此对话框，如果发生运行时错误，您至少需要向顶层过程添加异常处理，并在必要时在下层过程中也添加异常处理。</p><p>图1：包含的Continue按钮使 .NET 默认错误处理程序变得有些危险。</p><p>此外，其中的详细信息并不是您希望用户看到的内容。</p><h2 id="添加简单try-catch块处理异常" tabindex="-1"><a class="header-anchor" href="#添加简单try-catch块处理异常" aria-hidden="true">#</a> 添加简单try/catch块处理异常</h2><p>在C#中，为了恰当地处理运行时异常，在要需要保护的任何代码附近添加一个 try/catch块。在 try 块的代码中发生的任何运行时异常都将立即使用 catch 块中的代码继续执行：</p><div class="language-csharp" data-ext="cs"><pre class="language-csharp"><code><span class="token keyword">try</span>
...
<span class="token punctuation">}</span>
</code></pre></div><p>这段代码运行时，当有异常出现（如文件不存在）程序不会显示图2的对话框，取而代之的是一条简单的&quot;error occurred!&quot;警告，因为try块捕获住了异常并立即转到catch块中的代码继续执行。</p><p>注意：如果catch块中没有退出的代码（如return,throw），catch块后的代码将继续得到执行。并且try 块后面至少需要包含一个 catch 块（有关包含多个 catch 块的详细信息，请参阅下文）或一个finally块（参阅下文）。</p><h2 id="处理特定异常-多个catch块" tabindex="-1"><a class="header-anchor" href="#处理特定异常-多个catch块" aria-hidden="true">#</a> 处理特定异常（多个catch块）</h2><p>.NET 框架提供了大量的特定异常类，所有这些异常类都是从基类 Exception 类继承而来的。在 .NET 框架文档中，您会看到一些表，它们列出了在调用任何方法时都可能出现的所有异常。图 2便列出了.NET中File.Open()方法调用时可能发生的所有异常情况：</p><p>图 2： File.Open 可能发生的所有异常</p><p>你可以在一个try块后添加足够多的 Catch 块，以便对不同的异常情况作出不同的处理。以下的代码就列举了对几个不同的异常进行不同处理的情况。</p><div class="language-csharp" data-ext="cs"><pre class="language-csharp"><code><span class="token keyword">try</span>
<span class="token punctuation">{</span>
    <span class="token class-name"><span class="token keyword">string</span></span> filepath<span class="token operator">=</span><span class="token keyword">this</span><span class="token punctuation">.</span>textBoxfilepath<span class="token punctuation">.</span>Text<span class="token punctuation">;</span>
    <span class="token class-name"><span class="token keyword">long</span></span> isize<span class="token punctuation">;</span>
    <span class="token class-name">FileStream</span> fs<span class="token operator">=</span>File<span class="token punctuation">.</span><span class="token function">Open</span><span class="token punctuation">(</span>filepath<span class="token punctuation">,</span>FileMode<span class="token punctuation">.</span>Open<span class="token punctuation">)</span><span class="token punctuation">;</span>
...
<span class="token punctuation">{</span>
    MessageBox<span class="token punctuation">.</span><span class="token function">Show</span><span class="token punctuation">(</span><span class="token string">&quot;error occured!&quot;</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
<span class="token punctuation">}</span>
</code></pre></div><p>注意：catch块的次序必须十分小心，比如一个DivideByZeroException 异常继承自ArithmeticException异常，如果你先捕获后者，则当除数为0时抛出的异常就会进入ArithmeticException块而永远不会进入DivideByZeroException块。事实上，当这种情况出现时，编译器会发现DivideByZeroException块不能被执行到，并会报告一个编译错误。</p><h2 id="finally关键字" tabindex="-1"><a class="header-anchor" href="#finally关键字" aria-hidden="true">#</a> Finally关键字</h2><p>除了 try 和 catch 块中的代码外，有时你还需要添加无论何种情况下都会被执行到的代码，比如你可能需要释放一些资源、关闭一个文件等等，这是就需要使用Finally 关键字在 Catch 块后添加 Finally 块。即使代码抛出异常，并在Catch 块中添加了显式的return语句，finally块中的代码仍会被执行。Finally 块中的代码将在异常处理代码之后、控制返回到调用过程之前执行。</p><p>注意：finally 块只需要一个 try 块，catch 块的存在与否对其并没有影响。使用break,continue,return语句退出 finally 块都是非法的。</p><p>我们对上面的示例进行如下修改，以使任何情况下都可以调用结束代码，关闭可能打开了的文件：</p><div class="language-csharp" data-ext="cs"><pre class="language-csharp"><code><span class="token class-name">FileStream</span> fs<span class="token operator">=</span><span class="token keyword">null</span><span class="token punctuation">;</span>
...
<span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre></div><h2 id="throw-关键字" tabindex="-1"><a class="header-anchor" href="#throw-关键字" aria-hidden="true">#</a> Throw 关键字</h2><p>在C#中，要通知一个非正常情况，你可以使用 throw 关键字抛出一个异常。下面一行代码创建一个新的System.Exception实例，并将它抛出：</p><p><code>throw new System.Exception();</code></p><p>抛出的异常和所有自然引发的异常一样，立即将代码段挂起，并由CLR寻找一个异常处理者。如果要截取不同的异常并将它们作为单个异常类型全部返回到调用程序，使用 Throw 语句可以非常轻松地完成此操作。比如有一段代码捕获所有异常，而且无论导致异常的原因是什么，都只抛出一个 FileNotFoundException 对象给调用程序，因为实际的过程调用者可能并不关心实际发生的事情，也不关心为什么无法找到文件，他只关心该文件是否可用，并且需要从其他不同的异常中辨别该特定异常。</p><h2 id="exception对象" tabindex="-1"><a class="header-anchor" href="#exception对象" aria-hidden="true">#</a> Exception对象</h2><p>到目前为止我们一直使用Exception作为错误的信号，但未真正接触Exception对象本身，接着我们就来讨论这个问题。</p><p>图3：Exception 类的公共成员</p><p>图3是Exception类的公共成员变量。其中 Message 只读属性提供了关于这个异常的信息，比如为什么它被抛出，抛出异常的代码能在Exception构造函数中设定 Message 属性值。HelpLink 属性提供了一个此异常帮助文件的一个链接。而 StackTrace 只读属性是在运行时被设置的。在下面的例子中，Exception.HelpLink 属性被设置以提供用户关于DivideByZeroException的帮助，而异常的 StackTrace 属性提供一个对错误语句的运行栈追踪，显示了堆栈信息和导致异常抛出的一系列方法调用。</p><div class="language-csharp" data-ext="cs"><pre class="language-csharp"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">Test</span>
<span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre></div><div class="language-txt" data-ext="txt"><pre class="language-txt"><code>Output:
Open file here
...
Close file here.
</code></pre></div><p>在输出中，stack trace反向列出了被调用的方法，显示错误在DoDivide( )中发生，而此函数被TestFunc()调用。当多种方法纠缠在一起，stack trace能帮助你理清方法调用的次序。</p><p>在异常被抛出之前，你可以设置 HelpLink 属性：</p><p><code>e.HelpLink = &quot;http://www.libertyassociates.com&quot;;</code></p><p>这让你能为用户提供有用的信息。</p><h2 id="小结" tabindex="-1"><a class="header-anchor" href="#小结" aria-hidden="true">#</a> 小结</h2><ul><li>使用一个 Try/Catch 块可以向代码段添加异常处理。</li><li>.NET 运行时可以依次处理 Catch 块，它将使用找到的第一个匹配块。</li><li>您可以嵌套 Try 块，以便轻松而有效地推入和弹出异常处理状态。</li><li>在 Try 块后添加一个 Finally 块，这样无论发生什么，都可以无条件运行代码。</li></ul><p>作者：赵彦</p>`,47),e=[o];function c(u,l){return s(),a("div",null,e)}const k=n(p,[["render",c],["__file","summary12.html.vue"]]);export{k as default};
