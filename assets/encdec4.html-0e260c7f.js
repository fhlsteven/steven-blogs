...
<span class="token punctuation">{</span>
    ...
    <span class="token punctuation">{</span>
        ...
        <span class="token punctuation">{</span>
            ...
            <span class="token class-name">FileStream</span> fsIn <span class="token operator">=</span> File<span class="token punctuation">.</span><span class="token function">Open</span><span class="token punctuation">(</span>file<span class="token punctuation">,</span> FileMode<span class="token punctuation">.</span>Open<span class="token punctuation">,</span>
            FileAccess<span class="token punctuation">.</span>Read<span class="token punctuation">)</span><span class="token punctuation">;</span>
            <span class="token class-name">FileStream</span> fsOut <span class="token operator">=</span> File<span class="token punctuation">.</span><span class="token function">Open</span><span class="token punctuation">(</span>tempfile<span class="token punctuation">,</span> FileMode<span class="token punctuation">.</span>Open<span class="token punctuation">,</span>
            FileAccess<span class="token punctuation">.</span>Write<span class="token punctuation">)</span><span class="token punctuation">;</span>
            ...
            <span class="token class-name">CryptoStream</span> cstream <span class="token operator">=</span> <span class="token keyword">new</span> <span class="token constructor-invocation class-name">CryptoStream</span><span class="token punctuation">(</span>fsOut<span class="token punctuation">,</span> transform<span class="token punctuation">,</span>
            ryptoStreamMode<span class="token punctuation">.</span>Write<span class="token punctuation">)</span><span class="token punctuation">;</span>
            ...
            cstream <span class="token operator">=</span> <span class="token keyword">new</span> <span class="token constructor-invocation class-name">CryptoStream</span><span class="token punctuation">(</span>fsIn<span class="token punctuation">,</span> transform<span class="token punctuation">,</span>
            CryptoStreamMode<span class="token punctuation">.</span>Read<span class="token punctuation">)</span><span class="token punctuation">;</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre></div><h2 id="加密网络数据" tabindex="-1"><a class="header-anchor" href="#加密网络数据" aria-hidden="true">#</a> 加密网络数据</h2><p>如果我有一个只想自己看到的文档，我不会简单的通过e-mail发送给你。我将使用对称算法加密它；如果有人截取了它，他们也不能阅读该文档，因为他们没有用于加密的唯一密钥。但是你也没有密钥。我需要使用某种方式将密钥给你，这样你才能解密文档，但是不能冒密钥和文档被截取的风险。</p><p>非对称算法就是一种解决方案。这类算法使用的两个密钥有如下关系：使用公共密钥加密的信息只能被相应的私有密钥解密。因此，我首要求你给我发送你的公共密钥。在发送给我的途中可能有人会截取它，但是没有关系，因为他们只能使用该密钥给你的信息加密。我使用你的公共密钥加密文档并发送给你。你使用私有密钥解密该文档，这是唯一可以解密的密钥，并且没有通过网络传递。</p><p>不对称算法比对称算法计算的花费多、速度慢。因此我们不希望在线对话中使用不对称算法加密所有信息。相反，我们使用对称算法。下面的例子中我们使用不对称加密来加密对称密钥。接着就使用对称算法加密了。实际上安全接口层（SSL）建立服务器和浏览器之间的安全对话使用的就是这种工作方式。<br> 示例是一个TCP程序，分为服务器端和客户端。服务器端的工作流程是：</p><p>从客户端接收公共密钥。</p><p>使用公共密钥加密未来使用的对称密钥。</p><p>将加密了的对称密钥发送给客户端。</p><p>给客户端发送使用该对称密钥加密的信息。</p><p>代码如下：</p><div class="language-csharp" data-ext="cs"><pre class="language-csharp"><code><span class="token keyword">namespace</span> <span class="token namespace">com<span class="token punctuation">.</span>billdawson<span class="token punctuation">.</span>crypto</span>
<span class="token punctuation">{</span>
    ...
    <span class="token punctuation">{</span>
        ...
        <span class="token punctuation">{</span>
            ...
            <span class="token class-name">CryptoStream</span> cstream <span class="token operator">=</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre></div><p>客户端的工作流程是：</p><p>建立和发送公共密钥给服务器。</p><p>从服务器接收被加密的对称密钥。</p><p>解密该对称密钥并将它作为私有的不对称密钥。</p><p>接收并使用不对称密钥解密信息。</p><p>代码如下：</p><div class="language-csharp" data-ext="cs"><pre class="language-csharp"><code><span class="token keyword">namespace</span> <span class="token namespace">com<span class="token punctuation">.</span>billdawson<span class="token punctuation">.</span>crypto</span>
<span class="token punctuation">{</span>
    ...
    <span class="token punctuation">{</span>
        ...
        <span class="token punctuation">{</span>
            ...
            <span class="token class-name">CryptoStream</span> cstream <span class="token operator">=</span> <span class="token keyword">new</span> <span class="token constructor-invocation class-name">CryptoStream</span><span class="token punctuation">(</span>ms<span class="token punctuation">,</span> transform<span class="token punctuation">,</span>
            CryptoStreamMode<span class="token punctuation">.</span>Write<span class="token punctuation">)</span><span class="token punctuation">;</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre></div><h2 id="结论" tabindex="-1"><a class="header-anchor" href="#结论" aria-hidden="true">#</a> 结论</h2><p>使用对称算法加密本地数据时比较适合。在保持代码通用时我们可以选择多种算法，当数据通过特定的CryptoStream时算法使用转换对象加密该数据。需要将数据通过网络发送时，首先使用接收的公共不对称密钥加密对称密钥。</p><p>本文只涉及到<code>System.Security.Cryptography</code>名字空间的一部分服务。尽管文章保证只有某个私有密钥可以解密相应公共密钥加密的信息，但是它没有保证是谁发送的公共密钥，发送者也可能是假的。需要使用处理数字证书的类来对付该风险。</p><hr><hr><h2 id="对该文的评论-人气-3471" tabindex="-1"><a class="header-anchor" href="#对该文的评论-人气-3471" aria-hidden="true">#</a> 对该文的评论 人气：3471</h2><p>xiaorain (2003-11-3 10:07:47)</p><blockquote><p>客户端的工作流程是： 　　 建立和发送公共密钥给服务器。 　　 从服务器接收被加密的对称密钥。 　　 解密该对称密钥并将它作为&quot;&quot;私有的不对称密钥&quot;&quot;(不很明白)。 　　 接收并使用“”不对称密钥“”（什么意思，不是要用对称密钥吗？）解密信息。 还有，公钥和私钥是用户自己产生的吗？</p></blockquote><p>redpop (2003-10-31 23:29:02)</p><blockquote><p>very good !!!</p></blockquote><p>fa2002 (2003-10-31 11:16:27)</p><blockquote><p>to jinhailong:难道所有的书都是作者为自己写的日记吗？真是做技术做的脑子都坏掉了。</p></blockquote><p>fa2002 (2003-10-31 11:14:01)</p><blockquote><p>to jinhailong:知道的当然不用看了，不知道或是没用过加密技术的开发人员或是编程爱好者当然希望文章能清晰详实了。</p></blockquote><p>jinhailong (2003-10-31 9:58:00)</p><blockquote><p>呵呵，无所谓拉，反正做加密的要是连这个都不懂那还搞什么搞？例子很简洁，其实实际应用的话肯定不能这么做了。</p></blockquote><p>greystar (2003-10-30 12:11:48)</p><blockquote><p>TO Fa2002 真的有这种问题吗。我还没有试过。</p></blockquote><p>fa2002 (2003-10-29 14:07:05)</p><blockquote><p>不知道是水平问题还是别的原因，使用<code>symm.CreateEncryptor()</code>也不指定密钥和初始向量，代码没几行注释，而且还有错误。</p></blockquote>`,51),c=[o];function e(u,l){return s(),a("div",null,c)}const i=n(p,[["render",e],["__file","encdec4.html.vue"]]);export{i as default};
